    ...
    for key, triangle in reversed(triangles_dict.items()):
        ...
        if not is_delauney:
            ...
            common_points = [
                ...
                if point in adjecent_triangle.getPoints()
            ]
            p1 = [
                point for point in triangle.getPoints() if point not in common_points
            ][0]
            p2 = [
                point
                for point in adjecent_triangle.getPoints()
...
def _is_delauney(
    triangle: Triangle2D, triangles_dict: dict[str, Triangle2D]
) -> tuple[bool, str]:
    for p1, p2 in combinations(triangle.getPoints(), 2):
